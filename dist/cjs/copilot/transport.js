...
exports.CopilotTransport = void 0;
...
class CopilotTransport {
    ...
    constructor(opts) {
        ...
        this.GhClientCtor = opts.GhClientCtor ?? sdk_js_1.GhCopilotClient;
    }
    async start() {
        ...
        try {
            ...
            throw new errors_js_1.CopilotLaunchError(`Failed to instantiate Copilot SDK: ${err?.message ?? err}`);
        }
        try {
            if (this.config.resumeSessionId) {
                ...
                const sessionId = this.config.sessionId ?? (0, crypto_1.randomUUID)();
            ...
            if (/auth|token|credential/i.test(msg))
                throw new errors_js_1.CopilotAuthError(msg);
            throw new errors_js_1.CopilotLaunchError(msg);
        }
    }
    async stop() {
        if (this.gh) {
    ...
    checkUnsupportedFields() {
        const c = this.config;
        if (c.transport === 'pty') {
            throw new errors_js_1.CopilotFeatureUnsupportedError('transport', 'PTY transport is reserved for Phase 2 and not yet implemented.');
        }
        // SDK 0.3.0 does not support --mode or --max-autopilot-continues passthrough.
        if (c.mode !== undefined) {
            throw new errors_js_1.CopilotFeatureUnsupportedError('mode', '@github/copilot-sdk 0.3.0 does not yet support --mode passthrough; remove `mode` from config or upgrade.');
        }
        if (c.maxAutopilotContinues !== undefined) {
            throw new errors_js_1.CopilotFeatureUnsupportedError('maxAutopilotContinues', '@github/copilot-sdk 0.3.0 does not yet support --max-autopilot-continues passthrough.');
        }
        // SDK 0.3.0 SessionConfig has no allowAllTools / allowAllPaths / allowAllUrls / noAskUser fields.
        if (c.allowAllTools) {
            throw new errors_js_1.CopilotFeatureUnsupportedError('allowAllTools', '@github/copilot-sdk 0.3.0 does not support allowAllTools; use onPermissionRequest: approveAll instead.');
        }
        if (c.allowAllPaths) {
            throw new errors_js_1.CopilotFeatureUnsupportedError('allowAllPaths', '@github/copilot-sdk 0.3.0 does not support allowAllPaths; handle paths in onPermissionRequest.');
        }
        if (c.allowAllUrls) {
            throw new errors_js_1.CopilotFeatureUnsupportedError('allowAllUrls', '@github/copilot-sdk 0.3.0 does not support allowAllUrls; handle URLs in onPermissionRequest.');
        }
        if (c.noAskUser) {
            throw new errors_js_1.CopilotFeatureUnsupportedError('noAskUser', '@github/copilot-sdk 0.3.0 does not support noAskUser.');
        }
    }
    ...
    buildSessionConfig(sessionId) {
        ...
        const cfg = {
            ...
            onPermissionRequest: sdk_js_1.approveAll,
        };
        if (c.model)
            cfg.model = c.model;
...
exports.CopilotTransport = CopilotTransport;
