...
Add a `CopilotClient` to `@drunkcoding/ai-cli-clients` (renamed from `@baoduy2412/claude-client`) with similar — not identical — functionality to the existing `ClaudeClient`, by wrapping the official `@github/copilot-sdk`. In the same change, simplify `ClaudeClient` by collapsing the legacy `ClaudeClient` / `StructuredClaudeClient` two-layer split into a single class.
...
- All `@baoduy2412/claude-client` import strings in `examples/` updated to `@drunkcoding/ai-cli-clients`.
